...
class Unit:
    def __init__(self, x, y, max_move_points=11):
        self.x = x
        self.y = y
    ...
    def move_to(self, x, y, game_map):
        ...
        # BFS для поиска пути с учётом стоимости тайлов
        queue = deque()
        queue.append((self.x, self.y, self.move_points, self.used_negative, []))
        visited = {}
        ...
        while queue:
            ...
            if (cx, cy) == (x, y):
                target_path = path
                remaining_mp = mp_left
                remaining_neg = used_neg
                break
            for dx in [-1, 0, 1]:
                for dy in [-1, 0, 1]:
                    if dx == 0 and dy == 0:
        ...
        if target_path:
            # Устанавливаем путь для анимации
            self.path = target_path[1:]  # исключаем текущую позицию
            self.path_index = 0
            self.moving = True
            self.time_accumulator = 0
    ...
    def update(self, dt):
        """Обновление позиции юнита для анимации"""
        if self.moving and self.path_index < len(self.path):
            self.time_accumulator += dt
            if self.time_accumulator >= self.time_per_tile:
