...
class Unit:
    ...
    def _apply_type_stats(self):
        ...
        self.dice = cfg.get("dice", None)
        self.min_range = int(cfg.get("min_range", 1))
    ...
    def in_attack_range(self, tx: int, ty: int) -> bool:
        d = max(abs(int(tx) - int(self.x)), abs(int(ty) - int(self.y)))
        return self.min_range <= d <= self.range
